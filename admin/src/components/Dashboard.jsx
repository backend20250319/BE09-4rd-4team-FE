          ...
          <div key={index} className="p-6 bg-white border border-gray-100 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-sm font-medium text-green-600">{stat.change} 지난달 대비</p>
              </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="p-6 bg-white border border-gray-100 rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-sm text-[#9BCC47] hover:underline" onClick={()=> setCurrentPage('orders')}>모든 주문 보기</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="text-xs font-medium tracking-wider text-left text-gray-500 uppercase">
                  <th className="px-4 py-2">주문 번호</th>
                ...
                {recentOrders.map(order => (
                  <tr key={order.id} className="text-sm">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${order.status === '배송완료' ? 'bg-green-100 text-green-800' : order.status === '배송중' ? 'bg-blue-100 text-blue-800' : order.status === '결제완료' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}`}>{order.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="p-6 bg-white border border-gray-100 rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-sm text-[#9BCC47] hover:underline" onClick={() => setCurrentPage('products')}>모든 상품 보기</button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
              ...
              <tbody className="divide-y divide-gray-200">
                {topProducts.map(product => (
                  <tr key={product.id} className="text-sm">
                    ...
                    <td className="px-4 py-3 text-gray-700">{product.sales}</td>
