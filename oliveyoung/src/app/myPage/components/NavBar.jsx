  ...
  return (
    <nav className="w-52 border-r border-gray-200">
      ...
      {menuSections.map((section) => (
        <section
          key={section.title}
          className="mb-6 pt-[10px] pl-[10px] tracking-[-1px]"
        >
          <h2 className="text-base font-semibold mb-2.5 text-[16px]">
            {section.title}
          </h2>
              ...
              <li key={label} className="mb-2.5">
                <Link
                  ...
                  className={`cursor-pointer text-[13px] ${
                    pathname.startsWith(href) && href !== "#" ? "text-[#9bce26]" : "text-gray-800 hover:text-[#9bce26]"
                  }`}
                >
                  {label}
                </Link>
